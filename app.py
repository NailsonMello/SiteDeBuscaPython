...
def textoLinkScore(linhas, palavrasid):
    ...
    for id in palavrasid:
        cursor = conexao.cursor()
        cursor.execute(
            'select ul.idurl_origem, ul.idurl_destino from url_palavra up inner join url_ligacao ul on up.idurl_ligacao = ul.idurl_ligacao where up.idpalavra = %s',
            id)
...
#função não esta sendo utilizada
...
#eu coloquei uma função dentro da outra, mas poderia ser separada...
...
def send_data():
    ...
    def pesquisaPeso(consulta):
        ...
        dados = []
        title = []
        for (score, idurl) in scoresordenado[0:15]:
            # print('%f\t%s' % (score, getUrl(idurl)))
            pagina = urlopen(getUrl(idurl))
            res = BeautifulSoup(pagina.read(), "html5lib");
            tag = res.find("title", text=True)
            print(tag.string)
            print('%s' % getUrl(idurl))
            dados.append(getUrl(idurl))
            title.append(tag.string)
            #return dados, title


    pesquisaPeso(request.form['conteudoPesquisa'])
    #CUDUCOS = {'url': str(res),
               #'title': str(rugtt)}
    data = dados
    dats = title

    # rugtt = pesquisaPeso(request.POST.get('rugos'))
    return render_template('home.html', data=data, dats=dats)
